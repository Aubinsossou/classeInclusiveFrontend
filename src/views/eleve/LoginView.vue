<template>
  <div :class="['page', { 'high-contrast': forceHighContrast }]" role="main" aria-label="Page de connexion élève">
...
const numpadKeys = [
  ...
  { value:'clear',  label:'Effacer tout',                 type:'action-clear'  },
  { value:'0',      label:'0',                            type:'digit'         },
  { value:'delete', label:'Supprimer le dernier chiffre', type:'action-delete' },
]
...
async function handleSubmit() {
  ...
  try {
    ...
    router.push({ name: auth.isBlind ? 'BlindDashboard' : 'Dashboard' })
  } catch (err) {
...
.page.high-contrast .brand { color: #FFF; }
...
@keyframes blink {
  0%,100% { box-shadow: 0 0 0 0 rgba(92,79,224,.3); }
  50%      { box-shadow: 0 0 0 5px rgba(92,79,224,0); }
}
...
.error {
  ...
  font-size: .82rem; font-weight: bold; color: #B83232;
}
...
.key--digit:hover:not(:disabled) { background: #E8E4DC; border-color: rgba(120,100,80,0.22); }
...
.submit {
  ...
  background: #5C4FE0; color: #FFF; border: none; border-radius: 12px;
  font-family: 'Verdana', sans-serif; font-size: .92rem; font-weight: bold;
  cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px;
  margin-bottom: 10px; transition: all .15s;
  box-shadow: 0 3px 14px rgba(92,79,224,.22);
}
.submit:hover:not(:disabled) { background: #4A3ED0; transform: translateY(-1px); box-shadow: 0 5px 18px rgba(92,79,224,.32); }
...
.contrast-btn {
  ...
  background: transparent; color: #9C8E80;
  border: 1px solid rgba(120,100,80,0.13); border-radius: 10px;
  font-family: 'Verdana', sans-serif; font-size: .78rem;
