...
const needsConfirm = computed(() => isTDAH.value)
...
const questions = computed(() => {
  ...
  return quiz.questions.map((q) => ({
    ...
    correctIndex: (q.reponses || []).findIndex(
      (r) => r.status === 'correct' || r.is_correct || r.correct,
    ),
...
// FIX: pendingAnswer initialisé à -1 (jamais une option valide) pour éviter les faux positifs
...
const feedbackCorrectText = computed(() => (isSimplified.value ? '🎉 Bravo !' : 'Bonne réponse !'))
const feedbackWrongText = computed(() =>
  isSimplified.value ? 'Essaie encore la prochaine fois !' : 'Mauvaise réponse',
)

const getAnswerClass = (i) => {
  if (!answerGiven.value) {
    // FIX: pendingAnswer >= 0 pour éviter que -1 matche une option
    if (needsConfirm.value && pendingAnswer.value >= 0 && pendingAnswer.value === i)
      return 'answer-btn--pending'
    return selectedAnswer.value === i ? 'answer-btn--selected' : ''
  }
...
// FIX: Logique TDA/H corrigée
// Premier clic → met en pending
// Deuxième clic sur le même bouton → confirme
// Clic sur un autre bouton → change le pending
const selectAnswer = (i) => {
  if (answerGiven.value) return

  if (needsConfirm.value) {
    // Si aucun pending ou pending différent → on met en attente
    if (pendingAnswer.value !== i) {
      pendingAnswer.value = i
      return
    }
    // Si on clique à nouveau sur le pending → on confirme
    pendingAnswer.value = -1
  }
...
// FIX: Confirmation via bannière
const confirmPending = () => {
  if (pendingAnswer.value < 0) return
  const i = pendingAnswer.value
  pendingAnswer.value = -1
  selectedAnswer.value = i
  answerGiven.value = true
  answers.value[currentIndex.value] = i
...
const finishQuiz = () => {
  apiPost('/eleve/note/store', {
    note: score.value,
    eleve_id: eleve.value?.id,
    quiz_id: apiCours.value?.quizzes?.[0]?.id,
  })
  router.push({
    name: 'Result',
...
const handleKey = (e) => {
  if (['1', '2', '3', '4'].includes(e.key) && !answerGiven.value)
    selectAnswer(parseInt(e.key) - 1)
  if (e.key === 'ArrowRight' && answerGiven.value)
    isLast.value ? finishQuiz() : nextQuestion()
  // FIX: Entrée confirme le pending pour TDA/H
  if (e.key === 'Enter' && needsConfirm.value && pendingAnswer.value >= 0 && !answerGiven.value)
    confirmPending()
  if (e.key === 'Escape' && needsConfirm.value && pendingAnswer.value >= 0)
    cancelPending()
}
    ...
    <template v-else>
        ...
        <div class="header-inner">
          <button class="back-btn" @click="confirmExit" type="button">
            <svg viewBox="0 0 24 24" fill="none" width="18" height="18">
              <path d="M19 12H5M12 19l-7-7 7-7" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/>
            </svg>
            Quitter
          </button>
          <div class="pips-wrap">
            <div
              ...
              :class="['pip', { 'pip--current': i === currentIndex, 'pip--correct': answers[i] !== undefined && answers[i] === q.correctIndex, 'pip--wrong': answers[i] !== undefined && answers[i] !== q.correctIndex }]"
            ></div>
          </div>
      ...
      <div class="quiz-body">
        <div class="quiz-wrap">
          ...
          <!-- FIX: Bannière confirmation TDA/H — s'affiche quand pendingAnswer >= 0 -->
          <Transition name="feedback-slide">
            <div
              v-if="needsConfirm && pendingAnswer >= 0 && !answerGiven"
              class="confirm-banner"
              role="alert"
            >
              <p class="confirm-text">
                Es-tu sûr de ta réponse <strong>{{ letters[pendingAnswer] }}</strong> ?
              </p>
              <div class="confirm-btns">
                <button class="confirm-btn confirm-btn--yes" @click="confirmPending" type="button">
                  Oui, confirmer
                </button>
                <button class="confirm-btn confirm-btn--no" @click="cancelPending" type="button">
                  Changer
                </button>
              </div>
            </div>
          ...
          <Transition name="feedback-slide">
            ...
            >
              <div class="fb-icon">
                <svg v-if="feedbackState === 'correct'" viewBox="0 0 24 24" fill="none" width="22" height="22">
                  <circle cx="12" cy="12" r="10" fill="rgba(74,222,128,.2)" stroke="#4ADE80" stroke-width="2"/>
                  <path d="M8 12l3 3 5-5" stroke="#4ADE80" stroke-width="2.5" stroke-linecap="round"/>
                </svg>
                <svg v-else viewBox="0 0 24 24" fill="none" width="22" height="22">
                  <circle cx="12" cy="12" r="10" fill="rgba(248,113,113,.2)" stroke="#F87171" stroke-width="2"/>
                  <path d="M15 9l-6 6M9 9l6 6" stroke="#F87171" stroke-width="2.5" stroke-linecap="round"/>
                </svg>
              </div>
              <div class="fb-content">
                <strong class="fb-title">{{ feedbackState === 'correct' ? feedbackCorrectText : feedbackWrongText }}</strong>
          ...
          <fieldset class="answers-fieldset" :disabled="answerGiven">
            ...
            <div :class="['answers-grid', { 'answers-grid--motor': isMotor }]">
              <button
                v-for="(opt, i) in currentQuestion.options"
                :key="i"
                :class="['answer-btn', { 'answer-btn--motor': isMotor }, getAnswerClass(i)]"
                :disabled="answerGiven"
                @click="selectAnswer(i)"
                type="button"
              >
                <span :class="['ans-letter', { 'ans-letter--motor': isMotor }]">{{ letters[i] }}</span>
                <span class="ans-text">{{ opt }}</span>
                <span class="ans-indicator">
                  <svg v-if="answerGiven && i === currentQuestion.correctIndex" viewBox="0 0 24 24" fill="none" width="18" height="18">
                    <path d="M20 6L9 17l-5-5" stroke="#4ADE80" stroke-width="2.5" stroke-linecap="round"/>
                  </svg>
                  <svg v-else-if="answerGiven && i === selectedAnswer && i !== currentQuestion.correctIndex" viewBox="0 0 24 24" fill="none" width="18" height="18">
                    <path d="M18 6L6 18M6 6l12 12" stroke="#F87171" stroke-width="2.5" stroke-linecap="round"/>
                  </svg>
                </span>
              </button>
            </div>
          ...
          <!-- FIX: Hint clavier mis à jour pour TDA/H -->
          <p v-if="!isSimplified && !isMotor" class="kbd-hint">
            <kbd>1</kbd><kbd>2</kbd><kbd>3</kbd><kbd>4</kbd> pour sélectionner
            <template v-if="needsConfirm && pendingAnswer >= 0 && !answerGiven">
              · <kbd>Enter</kbd> pour confirmer · <kbd>Esc</kbd> pour changer
            </template>
            <span v-else-if="answerGiven"> · <kbd>→</kbd> pour continuer</span>
          </p>
        </div>
      </div>
    </template>
...
.quiz-header { background:#fdfbf8;border-bottom:1px solid rgba(120,100,80,.12);position:sticky;top:0;z-index:100; }
.header-inner { max-width:760px;margin:0 auto;padding:14px 24px;display:flex;align-items:center;gap:20px; }
.back-btn { display:flex;align-items:center;gap:6px;padding:8px 14px;background:transparent;color:#2c2416;border:1.5px solid rgba(120,100,80,.35);border-radius:10px;font-family:'Verdana','Geneva',sans-serif;font-size:.875rem;font-weight:bold;cursor:pointer;flex-shrink:0;transition:all .15s; }
.back-btn:hover { background:rgba(184,50,50,.06);color:#b83232;border-color:rgba(184,50,50,.3); }
.app--motor .back-btn { padding:12px 18px;font-size:1rem; }
...
.question-card { background:#fdfbf8;border:1px solid rgba(120,100,80,.12);border-radius:18px;padding:32px;outline:none; }
...
.app--simplified .q-text { font-size:1.8rem; }
.confirm-banner { background:rgba(245,158,11,.1);border:1.5px solid rgba(245,158,11,.4);border-radius:14px;padding:16px 20px; }
.confirm-text { font-size:1rem;font-weight:bold;color:#92400e;margin:0 0 12px; }
.confirm-btns { display:flex;gap:10px; }
.confirm-btn { padding:10px 20px;border-radius:10px;font-family:'Verdana','Geneva',sans-serif;font-size:.9rem;font-weight:bold;cursor:pointer;transition:all .15s;border:none; }
.confirm-btn--yes { background:#f59e0b;color:#fff; }
.confirm-btn--yes:hover { background:#d97706; }
.confirm-btn--no { background:#f0ede7;color:#2c2416;border:1px solid rgba(120,100,80,.2); }
...
.app--simplified .feedback-banner--wrong { background:rgba(245,158,11,.08);border-color:rgba(245,158,11,.25); }
.fb-content { flex:1; }
.fb-title { display:block;font-size:.95rem;font-weight:bold;margin-bottom:4px; }
.app--simplified .fb-title { font-size:1.2rem; }
.feedback-banner--correct .fb-title { color:#1a7a46; }
.feedback-banner--wrong .fb-title { color:#b83232; }
.app--simplified .feedback-banner--wrong .fb-title { color:#d97706; }
...
.answers-grid--motor { grid-template-columns:1fr;gap:14px; }
.answer-btn { width:100%;padding:18px 20px;background:#fdfbf8;color:#2c2416;border:1.5px solid rgba(120,100,80,.12);border-radius:14px;font-family:'Verdana','Geneva',sans-serif;font-size:.95rem;cursor:pointer;text-align:left;display:flex;align-items:center;gap:14px;transition:all .15s;min-height:72px; }
.answer-btn--motor { min-height:90px;font-size:1.05rem;padding:22px 24px; }
...
.answer-btn--pending { border-color:#f59e0b;background:rgba(245,158,11,.08);box-shadow:0 0 0 2px rgba(245,158,11,.25); }
...
.app--simplified .answer-btn--wrong { border-color:rgba(245,158,11,.5);background:rgba(245,158,11,.08); }
.answer-btn--dim { opacity:.4; }
.ans-letter { width:36px;height:36px;border-radius:10px;background:#f0ede7;border:1px solid rgba(120,100,80,.12);display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:.9rem;flex-shrink:0; }
.ans-letter--motor { width:48px;height:48px;font-size:1.1rem; }
...
.app--motor .nav-btn { padding:20px 36px;font-size:1.1rem; }
.nav-btn--next { background:#fdfbf8;color:#2c2416;border:1px solid rgba(120,100,80,.12); }
...
kbd { display:inline-flex;align-items:center;justify-content:center;padding:2px 8px;background:#f0ede7;border:1px solid rgba(120,100,80,.12);border-radius:6px;font-family:'Verdana','Geneva',sans-serif;font-size:.75rem;font-weight:bold; }
