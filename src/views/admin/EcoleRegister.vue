...
function getIconInner(name) {
  switch (name) {
    ...
    case 'phone':       return '<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.77 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.69a16 16 0 0 0 6.29 6.29l1.05-1.05a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>';
...
const form = reactive({
  name:            '',
  email:           '',
  numero:          '',   // type string — champ téléphone
  password:        '',
  confirmPassword: '',
})

const apiRegisterEcole = async () => {
  const response = await apiPost('ecole/register', {
    name:     form.name.trim(),
...
async function handleRegister() {
  ...
  // ── Validation ─────────────────────────────────────────────
  if (!form.name || !form.email || !form.numero || !form.password || !form.confirmPassword) {
  ...
  try {
    await apiRegisterEcole()
    // Succès → redirection vers le login
    router.push({ name: 'ecoleLogin' })

  } catch (err) {
    console.log('register error:', err)

    // Laravel renvoie les erreurs de validation dans err.response.data
    const data = err?.response?.data

    if (data?.errors) {
      // Erreurs de validation Laravel (422) — on affiche la première
      const firstField = Object.values(data.errors)[0]
      error.value = Array.isArray(firstField) ? firstField[0] : firstField

    } else if (data?.message) {
      // Message d'erreur général Laravel
        ...
        <form @submit.prevent="handleRegister" class="lc-form" novalidate>
          ...
          <div class="pro-field">
            ...
            <div class="lc-input-wrap">
              ...
              <input id="r-name" v-model="form.name" type="text"
                class="pro-input lc-input-padded"
                placeholder="Ex : École Sainte-Marie" required />
            </div>
          </div>
          ...
          <!-- Numéro de téléphone -->
          <div class="pro-field">
            <label for="r-numero" class="pro-label">Numéro de téléphone</label>
            <div class="lc-input-wrap">
              <AppIcon name="phone" :size="16" class="lc-input-icon" />
              <input id="r-numero" v-model="form.numero" type="text"
                class="pro-input lc-input-padded"
                placeholder="Ex : 0195959595" required />
            </div>
          </div>
...
.pro-btn-primary { background: var(--pro-blue); color: white; box-shadow: 0 2px 8px rgba(37,99,235,0.3); }
.pro-btn-primary:hover:not(:disabled) { background: #1d4ed8; box-shadow: 0 4px 16px rgba(37,99,235,0.4); transform: translateY(-1px); }
...
.login-left::before { content: ''; position: absolute; top: -80px; right: -80px; width: 320px; height: 320px; border-radius: 50%; background: rgba(255,255,255,0.04); }
.login-left::after  { content: ''; position: absolute; bottom: -60px; left: -60px; width: 240px; height: 240px; border-radius: 50%; background: rgba(255,255,255,0.03); }
...
.ll-mark { width: 52px; height: 52px; border-radius: 14px; background: rgba(255,255,255,0.12); color: white; display: flex; align-items: center; justify-content: center; }
...
.login-right { width: 100%; display: flex; align-items: center; justify-content: center; padding: 24px; overflow-y: auto; }
...
.lc-role-tag { display: inline-flex; align-items: center; gap: 6px; background: var(--pro-blue-soft); color: var(--pro-blue); border-radius: 100px; padding: 5px 12px; font-size: 0.72rem; font-weight: 700; margin-bottom: 14px; }
...
.lc-footer-links { display: flex; justify-content: center; align-items: center; gap: 6px; margin-top: 16px; }
.lc-link { display: inline-flex; align-items: center; gap: 4px; font-size: 0.85rem; font-weight: 700; text-decoration: none; transition: opacity 0.15s; }
