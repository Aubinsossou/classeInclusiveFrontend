...
const apiLoginEcole = async () => {
  const response = await apiPost('ecole/login', {
...
async function handleLogin() {
  ...
  try {
    const data = await apiLoginEcole()
    if (data.data.status === 'success') {
      localStorage.setItem('access_token', data.data.access_token)
      localStorage.setItem('role', data?.data.data?.roles[0].name)
      console.log('access_token:', data?.data?.access_token)
      console.log('role:', data?.data.data.roles[0].name)
      console.log('ecole connectée:', data.data)
    ...
    if (message === 'Aucune ecole trouver avec ce mail') {
      error.value = 'Aucun compte trouvé avec cette adresse e-mail.'
    } else if (message === 'Email ou mot de passe incorrect') {
  ...
  <div class="login-page">
    <div class="login-left">
      <div class="ll-content">
        ...
        <div class="ll-stats">
          <div class="ll-stat">
            <span class="ll-stat-n">10+</span><span class="ll-stat-l">Élèves</span>
          </div>
          <div class="ll-stat">
            <span class="ll-stat-n">5</span><span class="ll-stat-l">Enseignants</span>
          </div>
          <div class="ll-stat">
            <span class="ll-stat-n">4</span><span class="ll-stat-l">Classes</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="login-right">
      <div class="login-card">
        ...
        <form @submit.prevent="handleLogin" class="lc-form" novalidate>
          ...
          <button
            type="submit"
            class="pro-btn pro-btn-primary pro-btn-lg lc-submit"
            :disabled="isLoading"
          >
        ...
        <div class="lc-footer-links">
          <span style="color: var(--pro-muted); font-size: 0.85rem"
            >Vous n'avez pas de compte ?</span
          >
          <RouterLink
            :to="{ name: 'ecoleRegister' }"
            class="lc-link"
            style="color: var(--pro-blue)"
          >
            Créer un compte
          </RouterLink>
        </div>
