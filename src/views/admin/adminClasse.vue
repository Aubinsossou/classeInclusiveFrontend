...
const apiGEtListeClasse = async (ecole_id) => {
  loading.value = true
  listeClasse.value = userAuth.value?.data?.classes
  loading.value = false
}
...
<template>
  <div class="container">
    ...
    <div class="middle" v-if="listeClasse?.length > 0">
      <div class="card_class" v-for="Classe in listeClasse" :key="Classe.id">
        <!-- Header carte -->
        <div class="class_top">
        ...
        <div class="class_middle">
          ...
          <p>
            {{
              Classe?.enseignant
                ? Classe.enseignant.name + ' ' + Classe.enseignant.prenom
                : 'Aucun enseignant'
            }}
          </p>
        </div>
        ...
        <div class="class_low">
          <div class="stat_chip stat_chip_blue">
            ...
            <strong>{{ Classe?.eleves?.length ?? 0 }}</strong> élève{{
          ...
          <div class="stat_chip stat_chip_teal">
            ...
            <strong>{{ Classe?.matieres?.length ?? 0 }}</strong> matière{{
              (Classe?.matieres?.length ?? 0) > 1 ? 's' : ''
            }}
          </div>
        </div>
      </div>
    ...
    <div
      ...
      style="font-family: inherit; text-align: center; padding-top: 100px"
    >
  ...
  <div class="overlay" v-show="visible">
    <div class="contenu" v-if="classeEdit">
      ...
      <div class="low_overlay">
        <button class="btn_action btn_save" @click.prevent="apiClasseUpdate(classeUpdateId)">
          Enregistrer
        </button>
  ...
  <div class="overlay" v-show="visible_ajout">
    <div class="contenu">
      ...
      <div class="low_overlay">
        ...
        <button class="btn_action btn_cancel" @click.prevent="visible_ajout = false">
          Annuler
        </button>
      </div>
    </div>
  ...
  <div class="overlay" v-show="visible_matiere">
    <div class="contenu contenu_matiere">
      ...
      <div class="high_overlay">
        ...
        <p class="modal_sub">
          Classe : <strong>{{ classeSelectionnee?.name }}</strong>
        </p>
      </div>
      ...
      <div class="middle_overlay">
        ...
        <div class="matieres_grid">
          ...
          >
            ...
            {{ m.name }}
            <svg
              v-if="matieresSelectionnees.includes(m.id)"
              class="check_icon"
              width="13"
              height="13"
              viewBox="0 0 24 24"
      ...
      <div class="low_overlay">
        ...
        <button class="btn_action btn_cancel" @click.prevent="visible_matiere = false">
          Annuler
        </button>
      </div>
    </div>
...
.card_class {
  ...
  box-shadow: 0 4px 20px rgba(75, 191, 237, 0.1);
  border-radius: 16px;
  padding: 20px;
  gap: 12px;
  background: white;
...
.btn_save:hover:not(:disabled) {
  ...
  box-shadow: 0 4px 12px rgba(75, 191, 237, 0.4);
}
.btn_save:disabled {
  opacity: 0.5;
  cursor: not-allowed;
