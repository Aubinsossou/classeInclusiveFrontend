<template>
  <div class="page container">
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="h in filtered" :key="h.id">
            ...
            <td>
              <div style="display:flex;align-items:center;gap:8px">
                <strong style="font-size:.92rem">{{ h.studentsCount }}</strong>
                <div class="bar-track"><div class="bar-fill" :style="{ width: maxStudents ? (h.studentsCount/maxStudents*100)+'%' : '0%' }"></div></div>
              </div>
            </td>
            <td>
          ...
          <tr v-if="!filtered.length">
            <td colspan="3" style="text-align:center;padding:40px;color:#94a3b8;font-style:italic">Aucun profil trouvé.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- ══════════════════════════════════════════
         ...
         Teleport → sort du DOM du composant,
         se rattache directement à <body>
         → aucun parent ne peut casser le fixed
    ══════════════════════════════════════════ -->
    <Teleport to="body">
      <div v-if="showForm" class="h-overlay" @click.self="showForm=false">
        <div class="h-modal">
    ...
    ══════════════════════════════════════════ -->
    <Teleport to="body">
      <div v-if="showDelete" class="h-overlay" @click.self="showDelete=false">
        <div class="h-modal">
          <div class="h-modal-head">
...
const handicaps = ref([
  { id:1, name:'Déficience visuelle',           code:'VISUAL',   category:'sensory',       iconKey:'eye',  studentsCount:3,  active:true  },
  { id:2, name:'Déficience auditive',           code:'HEARING',  category:'sensory',       iconKey:'ear',  studentsCount:2,  active:true  },
  { id:3, name:'Déficience motrice',            code:'MOTOR',    category:'motor',         iconKey:'hand', studentsCount:2,  active:true  },
  { id:4, name:'Dyslexie',                      code:'DYSLEXIA', category:'cognitive',     iconKey:'edit', studentsCount:4,  active:true  },
  { id:5, name:'Troubles DYS (autres)',         code:'DYS',      category:'cognitive',     iconKey:'bolt', studentsCount:3,  active:true  },
  { id:6, name:'Troubles du spectre autistique',code:'TSA',      category:'psychological', iconKey:'user', studentsCount:1,  active:false },
])
...
const maxStudents = computed(() => Math.max(1, ...handicaps.value.map(h => h.studentsCount)))
...
function openAdd()  { editTarget.value = null; form.name = ''; showForm.value = true }
function openEdit(h){ editTarget.value = h;    form.name = h.name; showForm.value = true }
function submitForm() {
  if (!form.name.trim()) return
  if (editTarget.value) {
    const idx = handicaps.value.findIndex(h => h.id === editTarget.value.id)
    if (idx !== -1) handicaps.value[idx].name = form.name.trim()
  } else {
    handicaps.value.push({ id: Math.max(0,...handicaps.value.map(h=>h.id))+1, name:form.name.trim(), code:'', category:'sensory', iconKey:'user', studentsCount:0, active:true })
  }
...
function confirmDelete(h) { deleteTarget.value = h; showDelete.value = true }
function doDelete() {
  if (deleteTarget.value) handicaps.value = handicaps.value.filter(h => h.id !== deleteTarget.value.id)
  showDelete.value = false
}
</script>


<!-- styles scoped pour le tableau / page -->
<style scoped>
.page {
...
.btn-ghost { background:white; color:var(--sub); border-color:var(--border); }
...
<!--
  STYLES GLOBAUX pour les modales téléportées.
  Sans "scoped" → s'appliquent aux éléments injectés dans <body>
  par Teleport, qui sortent du scope du composant.
  Préfixe "h-" pour éviter les collisions avec d'autres styles globaux.
-->
<style>
.h-overlay {
  position: fixed;
...
.h-btn-danger:hover { background: #b91c1c; }
</style>