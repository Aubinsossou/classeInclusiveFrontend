...
const apiLoginEnseignant = async () => {
  const response = await apiPost('/enseignant/login', {
    email: form.email,
    password: form.password,
  })

  enseignantConnect.value = response

  if (enseignantConnect.value?.data?.status === 'success') {
    console.log('enseignantConnect.value: ', enseignantConnect.value)
    localStorage.setItem('access_token', enseignantConnect.value?.data?.access_token ?? '')
    localStorage.setItem('role', enseignantConnect.value.data?.data.roles[0]?.name ?? '')
    router.push('/enseignant/dashboard')
  } else {
    error.value = 'Une erreur est survenu veillez reessayer'
  }
}
...
async function handleLogin() {
  ...
  // // 2. Structuration des données (formData)
  // const formData = {
  //   email: form.email,
  //   password: form.password
  // }
    ...
    <div class="login-right">
      <div class="login-card">
        <div class="lc-header">
          <div
            class="lc-role-tag"
            style="background: var(--pro-green-soft); color: var(--pro-green)"
          >
        ...
        >
          ...
          <button
            type="submit"
            class="pro-btn pro-btn-green pro-btn-lg"
...
/* ============================================================================
   ...
   ============================================================================ */
/* 2. On attache les variables au conteneur principal pour respecter le "scoped" */
.login-page {
  --pro-blue: #2563eb;
  --pro-green: #059669;
