...
const svgPaths = {
  ...
  'list-ol':
    '<line x1="10" y1="6" x2="21" y2="6"/><line x1="10" y1="12" x2="21" y2="12"/><line x1="10" y1="18" x2="21" y2="18"/><path d="M4 6h1v4"/><path d="M4 10h2"/><path d="M6 18H4c0-1 2-2 2-3s-1-2-2-2"/>',
  'align-l':
    '<line x1="17" y1="10" x2="3" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="17" y1="18" x2="3" y2="18"/>',
  'align-c':
    '<line x1="21" y1="6" x2="3" y2="6"/><line x1="17" y1="10" x2="7" y2="10"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="17" y1="18" x2="7" y2="18"/>',
  'align-r':
...
const onDynamicInput = (listRef, index) => {
  if (index === listRef.length - 1 && listRef[index].trim() !== '') {
    listRef.push('')
  }
}

const removeDynamicItem = (listRef, index) => {
  listRef.splice(index, 1)
  if (listRef.length === 0 || listRef[listRef.length - 1].trim() !== '') {
    listRef.push('')
  }
}

const isLastEmptyItem = (listRef, index) => {
...
onMounted(async () => {
  try {
    ...
    if (isEdit) {
      const response = await apiGet(`enseignant/cours/show/${coursId}`)
      ...
      // FIX: utilise initDynamicList qui gère maintenant les objets {name}
      materielItems.value = initDynamicList(data.materiel ?? [])
      strategieItems.value = initDynamicList(data.strategie ?? [])
...
const submitForm = async (publish = null) => {
  ...
  // FIX: transformation strings → objets {name} attendus par le backend
  const materielList = materielItems.value
    .filter((i) => i.trim() !== '')
    .map((i) => ({ name: i.trim() }))

  const strategieList = strategieItems.value
    .filter((i) => i.trim() !== '')
    .map((i) => ({ name: i.trim() }))
  ...
  try {
    ...
    if (newMedias.value.length > 0) {
      ...
      // FIX FormData: envoie les objets {name} correctement
      materielList.forEach((item, i) => fd.append(`listMateriels[${i}][name]`, item.name))
      strategieList.forEach((item, i) => fd.append(`listStrategies[${i}][name]`, item.name))
      ...
      response = await apiPost(url, {
        ...
        listMateriels: materielList, // [{name: "Craie"}, {name: "Ardoise"}]
        listStrategies: strategieList, // [{name: "Ecrire avec la cler"}, ...]
      })
    }
...
function mediaIcon(type) {
  return { video: 'video', image: 'image', audio: 'audio' }[type] ?? 'upload'
}
function mediaColor(type) {
  return { video: '#3b82f6', image: '#10b981', audio: '#f59e0b' }[type] ?? '#94a3b8'
}
function mediaBg(type) {
  return { video: '#dbeafe', image: '#d1fae5', audio: '#fef3c7' }[type] ?? '#f1f5f9'
}
</script>
  ...
  <div class="cfv-root container">
    ...
    <template v-else-if="!error">
      <div v-if="!hasClasse" class="cfv-no-classe">
        ...
        Vous n'avez aucune classe assignée. Contactez l'administrateur de l'école avant de créer un
        cours.
      </div>
      ...
      <div class="cfv-header">
        ...
        <div class="cfv-header-center">
          ...
          <h1 class="cfv-title">
            {{ isEdit ? form.title || 'Modifier le cours' : 'Créer un cours' }}
          </h1>
        </div>
        ...
        <div class="cfv-main">
          ...
          <section class="cfv-card">
            ...
            <div class="field">
              ...
              <input
                v-model="form.title"
                class="pro-input"
                :class="{ 'pro-input-error': errors.title }"
            ...
            <div class="field">
              ...
              <textarea
                ...
                rows="2"
                placeholder="Un résumé bref du cours affiché dans la liste…"
              ></textarea>
            </div>
          </section>
          ...
          <section class="cfv-card">
            ...
            <div v-if="editor" class="tiptap-toolbar">
              ...
              >
                <span style="font-size: 0.75rem; font-weight: 800">H2</span>
              </button>
              <button
                class="tt-btn"
              ...
              >
                <span style="font-size: 0.75rem; font-weight: 800">H3</span>
              </button>
              <div class="tt-sep"></div>
              <button
                class="tt-btn"
          ...
          <section class="cfv-card">
            <h2 class="cfv-section-title">Résumé du cours</h2>
            <div class="field">
              <span v-if="errors.title" class="field-error">{{ errors.title }}</span>
            </div>
            <div class="field">
          ...
          <section class="cfv-card">
            ...
            <p class="cfv-section-sub">
              Vidéo prioritaire comme miniature. Formats supportés : MP4, JPG/PNG, MP3/WAV. Max 100
              MB par fichier.
            </p>

            <div v-if="existingMedias.length" class="medias-list">
              <div
                v-for="m in existingMedias"
                :key="m.id"
                class="media-item"
                :style="{ '--mc': mediaColor(m.type), '--mb': mediaBg(m.type) }"
              >
                ...
                <div class="media-info">
                  <span class="media-type-badge"
                    ><AppIcon :name="mediaIcon(m.type)" :size="11" /> {{ m.type }}</span
                  >
                  <span class="media-url">{{ m.url.split('/').pop() }}</span>
                </div>
                <button
                  class="media-del-btn"
                  @click="deleteExistingMedia(m)"
            ...
            <div v-if="newMedias.length" class="medias-list" style="margin-top: 10px">
              <div
                v-for="(m, i) in newMedias"
                :key="i"
                class="media-item media-item-new"
                :style="{ '--mc': mediaColor(m.type), '--mb': mediaBg(m.type) }"
              >
                <div class="media-preview">
                  <video
                  ...
                  />
                  <img
                    v-else-if="m.type === 'image' && m.preview"
                    :src="m.preview"
                    class="media-preview-el"
                  />
                  <div v-else class="media-preview-audio">
                    <AppIcon :name="mediaIcon(m.type)" :size="22" />
                  </div>
                </div>
                <div class="media-info">
                  <span class="media-type-badge"
                    ><AppIcon :name="mediaIcon(m.type)" :size="11" /> {{ m.type }} — nouveau</span
                  >
                  <span class="media-url">{{ m.name }}</span>
                  <span class="media-size">{{ m.size }}</span>
                </div>
                <button class="media-del-btn" @click="removeNewMedia(i)">
            ...
            <div class="media-add-row">
              <label class="media-add-btn">
                ...
                <input
                  ...
                  multiple
                  hidden
                  @change="(e) => handleFilePick(e, 'video')"
                />
              </label>
              <label class="media-add-btn">
                ...
                <input
                  ...
                  multiple
                  hidden
                  @change="(e) => handleFilePick(e, 'image')"
                />
              </label>
              <label class="media-add-btn">
                ...
                <input
                  ...
                  multiple
                  hidden
                  @change="(e) => handleFilePick(e, 'audio')"
                />
              </label>
            </div>
          </section>
          ...
          <section class="cfv-card">
            ...
            <p class="cfv-section-sub">Listez les matériels dont les élèves ont besoin</p>
            <div class="dynlist">
              <div v-for="(item, index) in materielItems" :key="index" class="dynlist-row">
                <input
          ...
          <section class="cfv-card" style="padding-bottom: 30px">
            <h2 class="cfv-section-title">Stratégie d'utilisation</h2>
            <p class="cfv-section-sub">
              Décrivez étape par étape comment exploiter ces stratégies en classe.
            </p>
            <div class="dynlist">
              <div v-for="(item, index) in strategieItems" :key="index" class="dynlist-row">
                <textarea
        ...
        <aside class="cfv-aside">
          <section class="cfv-card">
            ...
            <div class="field">
              ...
              <select
                ...
                class="pro-input"
                :class="{ 'pro-input-error': errors.matiere_id }"
              >
            ...
            <div class="field">
              ...
              <div v-if="myClasse" class="classe-display">
                <span class="classe-name" style="text-transform: uppercase">{{
                  myClasse.name
                }}</span>
              </div>
            ...
            <div class="field">
              ...
              <div class="toggle-row">
                ...
                <button
                  type="button"
                  class="toggle-btn"
                  :class="{ 'toggle-on': form.is_published }"
          ...
          <section class="cfv-card" v-if="existingMedias.length || newMedias.length">
            ...
            <div class="media-summary">
              <div
                class="ms-row"
                v-if="[...existingMedias, ...newMedias].filter((m) => m.type === 'video').length"
              >
                <div class="ms-icon" style="background: #dbeafe; color: #3b82f6">
                  <AppIcon name="video" :size="13" />
                </div>
                <span
                  >{{
                    [...existingMedias, ...newMedias].filter((m) => m.type === 'video').length
                  }}
                  vidéo(s)</span
                >
              </div>
              <div
                class="ms-row"
                v-if="[...existingMedias, ...newMedias].filter((m) => m.type === 'image').length"
              >
                <div class="ms-icon" style="background: #d1fae5; color: #10b981">
                  <AppIcon name="image" :size="13" />
                </div>
                <span
                  >{{
                    [...existingMedias, ...newMedias].filter((m) => m.type === 'image').length
                  }}
                  image(s)</span
                >
              </div>
              <div
                class="ms-row"
                v-if="[...existingMedias, ...newMedias].filter((m) => m.type === 'audio').length"
              >
                <div class="ms-icon" style="background: #fef3c7; color: #f59e0b">
                  <AppIcon name="audio" :size="13" />
                </div>
                <span
                  >{{
                    [...existingMedias, ...newMedias].filter((m) => m.type === 'audio').length
                  }}
                  audio(s)</span
                >
              </div>
          ...
          <section class="cfv-card">
            ...
            <button
              class="pro-btn pro-btn-ghost"
              style="width: 100%; justify-content: center; margin-top: 8px; color: var(--pro-muted)"
              @click="goBack"
              :disabled="saving"
            >
              Annuler
...
.tiptap-toolbar {
  ...
  border: 1.5px solid var(--pro-border);
  border-bottom: none;
  border-radius: var(--pro-r-md) var(--pro-r-md) 0 0;
  background: var(--pro-bg);
}
.tt-btn {
  display: inline-flex;
...
.tiptap-wrap {
  border: 1.5px solid var(--pro-border);
  border-radius: 0 0 var(--pro-r-md) var(--pro-r-md);
  min-height: 200px;
}
:deep(.tiptap-area) {
  padding: 14px 16px;
...
.media-del-btn:hover:not(:disabled) {
  background: var(--pro-red-soft);
  color: var(--pro-red);
  border-color: #fecaca;
}
.media-del-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.media-add-row {
